...
if [ -z "$DATASET" ]; then
    echo "Usage: bash run_feature_extraction.sh [training|simulation|real_biological|custome]"
    exit 1
fi
...
elif [ "$DATASET" == "custome" ]; then
    METADATA="metadata/custome_metadata.tsv"
    TREES_DIR="dataset/custome_dataset"
    OUTPUT_DIR="results/custome_features"
    OUTPUT_FILE="results/custome_results.tsv"
else
...
PYTHON_SCRIPT="scripts/feature_extraction.py"
...
# Process trees
tail -n +2 "$METADATA" | while IFS=$'\t' read -r id prot donor receptor format pattern; do
    tree_file="${TREES_DIR}/${id}.nexus"
    output_file="${OUTPUT_DIR}/${id}.tsv"
